...
class CoordinationKivyApp(App):
    ...
    def on_select(instance):
      if filechooser.selection:
        ...
        if file_type == "excel":
          ...
          self.excel_status.text = ar(
              f"تم اختيار: {os.path.basename(selected)}"
          )
          self.load_schools()
        elif file_type == "logo":
          self.logo_path = selected
          self.logo_status.text = ar(
              f"تم اختيار اللوجو: {os.path.basename(selected)}"
          )
      popup.dismiss()
  ...
  def load_schools(self):
    try:
      ...
      for r in range(2, ws_schools.max_row + 1):
        val = (
            ws_schools.cell(row=r, column=col_school_title_idx).value
            if col_school_title_idx
            else None
        )
      ...
      for sch_name in unique_schools:
        ...
        cap_tf = TextInput(
            text="45", multiline=False, input_filter="int", size_hint_x=0.3
        )
      ...
      self.status_txt.text = ar(
          f"تم تحميل عدد ({len(unique_schools)}) مدرسة بنجاح."
      )
  ...
  def start_coordination(self, instance):
    ...
    try:
      calc_date = datetime(
          int(year_str), int(self.month_tf.text), int(self.day_tf.text)
      )
    except:
    ...
    self.status_txt.text = ar(
        f"جاري الفرز والتسكين لطلاب المرحلة {stage_arabic}..."
    )

    output_dir = os.path.dirname(self.excel_path)
    output_file = os.path.join(
        output_dir, f"منظومة_التنسيق_المرحلة_{stage_arabic}.xlsx"
    )
    pdf_folder = os.path.join(
        output_dir, f"كشوف_المدارس_المرحلة_{stage_arabic}_PDF"
    )
    ...
    col_student_name_idx = find_col_idx(
        lambda h: "اسم الطالب" in h or ("الاسم" in h and "المدرسة" not in h),
        default=2,
    )
    ...
    col_r4_name_idx = find_col_idx(
        lambda h: "المدرسة المتميزة" in h and "كود" not in h
    )
    ...
    for sch_name, (cap_tf, age_tf) in self.school_inputs.items():
      ...
      if stage_num > 1:
        alloc_dobs = [
            s["dob_dt"]
            for s in students
            if self.clean_school_name(s["out_name"]) == c_name
            and s["dob_dt"] is not None
        ]
        if alloc_dobs:
    ...
    for st in students:
      ...
      if (
          stage_num > 1
          and curr_alloc
          and curr_alloc not in ["قائمة الانتظار", "nan", ""]
      ):
    ...
    students_sorted = sorted(
        students,
        key=lambda x: x["dob_dt"] if x["dob_dt"] is not None else datetime.max,
    )
    ...
    for st in students_sorted:
        ...
        if sch_name_str and sch_name_str not in ["لا يوجد", "nan"]:
          ...
          if school_capacities[sch_name_str] > 0:
            ...
            break
              ...
              school_capacities[sch_name_str] == 0
              and school_last_dob[sch_name_str] == dob_dt
              and dob_dt is not None
          ):
            st["out_name"] = sch_name_str
            st["out_code"] = sch_code
            school_accepted_count[sch_name_str] += 1
      ...
      if not allocated:
        ...
        st["out_code"] = 0
        st["notes"] = (
            "استنفاذ رغبات اقل من السن المحدد"
            if rejected_by_age
            else "استنفاذ رغبات"
        )
    ...
    for sch_title, st_list in grouped_students.items():
      safe_filename = (
          f"كشف_{sch_title.replace(' ', '_')}_المرحلة_{stage_arabic}.pdf"
      )
      pdf_out_path = os.path.join(pdf_folder, safe_filename)
      self.generate_pdf_report(
          sch_title, st_list, pdf_out_path, stage_arabic, calc_date
      )
